...
const App: React.FC = () => {
  ...
  const [isAuthMode, setIsAuthMode] = useState<'login' | 'register' | null>('login');
  ...
  const handleAuth = (e: React.FormEvent) => {
    ...
    if (isAuthMode === 'register') {
      if (users.length >= MAX_USERS) {
        alert('El grupo familiar está lleno (máx 6).');
        return;
      }
      if (users.find(u => u.name === authForm.name)) {
      ...
      const newUser: User = {
        ...
        avatar: `https://api.dicebear.com/7.x/avataaars/svg?seed=${authForm.name}`,
        moneySpent: 0,
        gamesCount: 0,
        games: []
      };
      ...
      const user = users.find(u => u.name === authForm.name && u.password === authForm.password);
      if (user) {
        setCurrentUser(user);
        setIsAuthMode(null);
      } else {
        ...
        <div className="bg-[#2a475e] p-8 rounded-2xl w-full max-w-md shadow-2xl border border-blue-900/30">
          ...
          <form onSubmit={handleAuth} className="space-y-4">
            <input 
              ...
              className="w-full bg-black/30 border border-gray-700 rounded-lg p-3 text-white outline-none focus:ring-2 focus:ring-blue-500"
              value={authForm.name}
            ...
            />
            <button className="w-full bg-blue-600 hover:bg-blue-500 text-white py-3 rounded-lg font-bold transition-all uppercase tracking-widest">
              {isAuthMode === 'login' ? 'Iniciar Sesión' : 'Registrarse'}
            </button>
          </form>
          <button 
          ...
          >
            {isAuthMode === 'login' ? '¿No tienes cuenta? Regístrate' : '¿Ya tienes cuenta? Inicia sesión'}
          </button>
        </div>
      </div>
    );
